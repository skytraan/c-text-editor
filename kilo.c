...
void enableRawMode(){
    ...
    atexit(disableRawMode); /* disables terminal settings after program end */
    ...
    raw.c_cc [VTIME] = 1; // max time before read() returns

    if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1) die("tcsetattr");
}

/* Waits for one keypress & returns it */
int editorReadKey() {
    int nread;
    char c;
...
int getWindowsSize(int *rows, int *cols){
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0){
        ...
        /* on success, places num of columns wide, rows high the terminal is
        into winsize struct */
        *cols = ws.ws_col;
        *rows = ws.ws_row;
        return 0;
            ...
            for (j = 0; j<len; j++){
                if (hl[j] == HL_NORMAL){
                    ...
                    if (color != current_color){
                        current_color = color;
                        char buf[16];
                        int clen = snprinf(buf, sizeof(buf), "\x1b[%dm", color);
